      ...
      <main className="pt-20">
        <div className="section-padding">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            >
              {product.badge && (
                <span className={`inline-block w-fit mb-4 ${
              ...
              <h1 className="text-2xl md:text-4xl font-display uppercase tracking-wider mb-2">{product.name}</h1>
              <p className="text-xl font-display text-primary mb-6">€{product.price.toFixed(2)}</p>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.sizes.map(size => (
                    <button
                      ...
                      className={`px-4 py-2 border text-xs font-display uppercase tracking-wider transition-all ${
                        selectedSize === size
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center gap-4 border border-border w-fit">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 py-2 text-foreground/50 hover:text-foreground">
                    <Minus size={16} />
                  </button>
                  <span className="font-body text-sm min-w-[2rem] text-center">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="px-3 py-2 text-foreground/50 hover:text-foreground">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
              ...
              <button
                ...
                className={`btn-primary w-full md:w-auto text-center ${!selectedSize ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                {t("shop.addtocart")}
              </button>
          ...
          {related.length > 0 && (
            <div className="mt-24">
              <h2 className="text-xl md:text-2xl font-display uppercase tracking-[0.2em] mb-8">{t("shop.related")}</h2>
